...
`GET /api/v1/merchants/:merchant_id/revenue`
 ...
 I was excited to try and use a module in this project, and inheritance if possible. I wasn't able to implement any inheritance, but I did create a module to contain the crazy methods I wrote to convert cents to dollars. I had originally tried to use a module to remove the repetition from the search services, but upon creation I realized that I no longer needed two separate searches anyways and just replaced them with a single dynamic service. 
 
 The module for converting dollars to cents is called Dollarable and is located in `lib/assets` which Ian (instructor) indicated is the appropriate location since we could potentially recycle a module housed in this location and use it in other ruby or rails projects. This instruction helped me understand that our modules should be so dynamic that we could plop them in another project and they would still work. Mind. Blown.
 
 In the future I would be interested to know if the repetition in the MerchantsController and ItemsController could be eliminated through inheritance. I understand that it wouldn't be through ApplicationController since all Controllers inherit from that, but I could create a BaseController that inherited from ApplicationController and then setup the Merchants and Items controllers to inherit from that. 
